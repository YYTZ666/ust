...
system_ids.long_log = system.runInterval(() => {
  ...
  for (var name of Object.keys(log_info.bb)) {
    for (var id of Object.keys(log_info.bb[name])) {
      ...
      server_log(0, text, get_player_path({
        name: name
      }))
    }
  }
...
function afterEntityHitEntity(event) {
  ...
  if (is_player(hitter)) {
    if (entity.typeId === "minecraft:npc") {
      ...
      if (data !== "") {
        system.run(() => {
          showConfigBar(hitter, to_object(parse_json(data)))
        })
        event.cancel = true
      }
    }
...
function beforePlayerInteractWithBlock(event) {
  ...
  if (is_player(player)) {
    if (!un(item)) {
      //system.run(()=>{mc.world.sendMessage("test1")});
    } else {
      //system.run(()=>{mc.world.sendMessage("test2")});
...
function parse_location_text(str, none = 0) {
  if (str.startsWith("~")) {
    if (str === "~") {
      return none
    } else {
      return none + to_number(parseFloat(str.slice(1)))
    }
  } else {
    return to_number(parseFloat(str), none)
  }
}
...
function tpaRequest(goal, player, mode) {
  ...
  ui.busy = 0 //只要busy不是function就会一直尝试弹出窗口
  ui.show(goal)
}
